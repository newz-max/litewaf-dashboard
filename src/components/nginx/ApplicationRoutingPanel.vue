...
import ApplicationRoutingEditorModal from "@/components/nginx/ApplicationRoutingEditorModal.vue"
import { hasApplicationProxyConfig } from "@/utils/applicationProxyConfig"
...
const applicationColumns = computed(() => [
  ...
  {
    title: t("nginxProxy.routing.routes"),
    key: "routes",
    minWidth: 160,
    render: (row: Application) => summarizeRoutes(row)
  },
  {
...
function summarizeUpstreams(application: Application) {
  return (
    application.upstreams
      ...
      .map((upstream) => upstream.name ? `${upstream.name}: ${upstream.url}` : upstream.url)
      .join(", ") || t("common.noData")
  )
}

function summarizeRoutes(application: Application) {
...
async function saveApplicationRouting(payload: ApplicationInput) {
...
<template>
  <section class="section section-pad">
    ...
    <ApplicationRoutingEditorModal
      v-model:show="showModal"
      :application="selectedApplication"
      :saving="saving"
      :error="saveError"
      @save="saveApplicationRouting"
